package net.togogo.egou.common.utils;
...
public class PageController {
  ...
  public PageController(int totalCount,int currentPage) {
    setPageController(totalCount,currentPage);
  }
  ...
  public void setPageController(int totalCount,int currentPage){
   ...
   * @param totalCount 总行数。
   */
  private void setTotalRowsAmount(int totalCount) {

    if(totalCount <0 ){
      totalRowsAmount =  0;
    }else{
      totalRowsAmount = totalCount;
    }

    if(totalRowsAmount%pageSize==0){
      totalPages = (int) (totalRowsAmount / pageSize);
    }else{
      totalPages = (int) (totalRowsAmount / pageSize + 1);
    }
  }
  ...
  public void setCurrentPage( int curPage) {
    ...
    if(currentPage != totalPages){
      ...
      pageStartRow = (currentPage - 1)*pageSize;
      //记录索引从0开始
      pageEndRow = (int) (totalRowsAmount - pageStartRow);
    }
  }
